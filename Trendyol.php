<?php
require_once TRENDPRESS_PLUGIN_DIR."vendor/autoload.php";
use Curl\Curl;
...
class Trendyol
{
    ...
    public function by_seller($path)
    {
        while ($this->whileCheck) {
            if ($this->whileCounter == 0) {
                $this->url = $this->base_url . $this->api_v2 . "filter/" . $path;
            } else {
                $this->url = $this->base_url . $this->api_v2 . "filter/" . $path . "&pi=" . $this->whileCounter;
            }

            if ($this->whileCounter > 100) {
            ...
            $this->whileCounter++;
            if ($this->whileCounter >= ($this->curl->response->result->totalCount/25)){
                $this->whileCheck = false;
            }
        }
